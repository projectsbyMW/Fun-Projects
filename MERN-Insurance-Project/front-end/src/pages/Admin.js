...
const Admin = () => {
  ...
  const [loading, setLoading] = useState(true); // Add loading state to force rerender

  // Fetch data function
  const fetchData = async () => {
    try {
      setLoading(true); // Set loading state to true when fetching data
      const documentResponse = await axios.get('https://fun-projects.onrender.com/documents');
      setDocuments(documentResponse.data);
      const employeeResponse = await axios.get('https://fun-projects.onrender.com/employees');
  ...
  const handleDocumentStatusUpdate = async (id) => {
    try {
      ...
      await axios.put(`https://fun-projects.onrender.com/documents/${id}`, { status: statusToUpdate, rejectionReason: reason });

      // After successful update, fetch data again to force refresh
      await fetchData();

      // Reset input fields for the updated document
  ...
  const handlePolicyApproval = async (employeeId) => {
    try {
      await axios.put(`https://fun-projects.onrender.com/employees/${employeeId}/policy`, { policyStatus });
